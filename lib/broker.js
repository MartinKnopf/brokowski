...
Brokowski.prototype.on = function(event, cb) {  
  this.events[event] = this.events[event] || [];
  this.events[event].push(cb);
};


Brokowski.prototype.emit = function(event, data, http) {
  for(var i = 0, len = this.events[event].length; i < len; i++) {
    this.events[event][i](data, http);
  }
};


Brokowski.prototype.removeAllListeners = function() {
  this.events = {};
};
