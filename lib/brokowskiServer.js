var http = require('http')
  , url = require('url')
  , broker = require('./brokowskiRoutes').routes();
...
BrokowskiServer.prototype.start = function(port) {
  ...
  this.server.listen(this.port || port || 3000, function() {
...
BrokowskiServer.prototype.POST = function(req, res) {
  ...
  req.on('end', function() {
    // broker.publish('event-name', data) or broker.subscribe('event-name', data)
    res.statusCode = broker[uri[1]](uri[2], data) || 200;
    res.end();
  });
}
...
BrokowskiServer.prototype.GET = function(req, res) {
  ...
  res.statusCode = broker[uri[1]](uri[2]); // broker.monitoring('alive');
  res.end();
}